...
std::optional<std::vector<int64_t>> GraphShapePass::GetTensorVariableShape(
    uint32_t var_id) const {
  ...
  if (constant->AsNullConstant() != nullptr) {
    ...
    if (isSplat) {
      if (components.size() != 1) {
        assert(false &&
               "Unexpected number of elements in TensorConstant (splat)");
        return {};
      }
      result.resize(numElements, components[0]->GetS32());
    } else {
