...
std::vector<uint64_t> ShapeGraphInstructionArgmaxOp(IRContext* context,
                                                    uint32_t input_id,
                                                    uint32_t axis_id) {
...
std::vector<uint64_t> ShapeGraphInstructionAvgMaxPool2dOps(IRContext* context,
                                                           uint32_t kernel_id,
                                                           uint32_t stride_id,
                                                           uint32_t pad_id,
                                                           uint32_t input_id) {
  ...
  uint64_t OH = idiv_check(IH + pad_top + pad_bottom - kernel_y, stride_y) + 1;
  uint64_t OW = idiv_check(IW + pad_left + pad_right - kernel_x, stride_x) + 1;

  return {N, OH, OW, C};
}

std::vector<uint64_t> ShapeGraphInstructionMatchOutput(IRContext* context,
                                                       uint32_t input_id) {
  auto input_shape = *GetTensorShape(context, input_id);
  return input_shape;
}

std::vector<uint64_t> ShapeGraphInstructionMatchOutputWithCheck(
...
std::vector<uint64_t> ShapeGraphInstructionReduceOps(IRContext* context,
                                                     uint32_t input_id,
                                                     uint32_t axis_id) {
  auto axis = context->get_constant_mgr()
                  ->FindDeclaredConstant(axis_id)
...
std::vector<uint64_t> ShapeGraphInstructionSelectOp(IRContext* context,
                                                    uint32_t input2_id,
                                                    uint32_t input3_id) {
  auto input2_shape = *GetTensorShape(context, input2_id);
...
std::vector<uint64_t> ShapeGraphInstructionConv2dOp(
  ...
  uint64_t OH =
      idiv_check(IH - 1 + pad_top + pad_bottom - (KH - 1) * dilation_y,
                 stride_y) +
      1;
  uint64_t OW =
      idiv_check(IW - 1 + pad_left + pad_right - (KW - 1) * dilation_x,
                 stride_x) +
      1;
...
std::vector<uint64_t> ShapeGraphInstructionResizeOp(IRContext* context,
                                                    uint32_t input_id,
                                                    uint32_t scale_id,
                                                    uint32_t offset_id,
                                                    uint32_t border_id) {
...
std::vector<uint64_t> ShapeGraphInstructionConcatOp(IRContext* context,
                                                    Instruction* inst,
                                                    uint32_t axis_id,
